...
{
  rDebug("Initializing RA procedure");
  std::cout << "Initializing Random Access Procedure" << std::endl;
...
{
  rDebug("Selecting RA resources");
  std::cout << "Selecting Random Access Resources" << std::endl;
  ...
  std::cout << "Selected preambleIndex=" << sel_preamble.load() 
            << " maskIndex=" << sel_maskIndex 
            << " GroupA=" << rach_cfg.nof_groupA_preambles 
            << " GroupB=" << nof_groupB_preambles << std::endl;
...
void ra_proc::preamble_transmission()
{
  ...
  rDebug("Transmitting PRACH preamble with power=%d dBm", received_target_power_dbm);
  std::cout << "Transmitting PRACH preamble with power=" << received_target_power_dbm << " dBm" << std::endl;
...
void ra_proc::process_timeadv_cmd(uint32_t tti, uint32_t ta)
{
  if (preambleIndex == 0) {
    ...
    logger.info("Timing Advance Command received: TA=%d, TTI=%d", ta, tti);
    std::cout << "Timing Advance Command received: TA=" << ta << ", TTI=" << tti << std::endl;
  } else {
...
void ra_proc::new_grant_dl(mac_interface_phy_lte::mac_grant_dl_t grant, mac_interface_phy_lte::tb_action_dl_t* action)
{
  ...
  if (grant.tb[0].tbs < MAX_RAR_PDU_LEN) {
    ...
    std::cout << "Received DL grant for RA-RNTI=" << ra_rnti << std::endl;
...
void ra_proc::tb_decoded_ok(const uint8_t cc_idx, const uint32_t tti)
{
  ...
  rDebug("RAR PDU received, attempting to decode...");
  std::cout << "RAR PDU received, attempting to decode..." << std::endl;
  ...
  if (rar_pdu_msg.parse_packet(rar_pdu_buffer) != SRSRAN_SUCCESS) {
    ...
    std::cout << "Error decoding RAR PDU" << std::endl;
    return;
  }

  rDebug("RAR decoded successfully TBS=%d", rar_grant_nbytes);
  std::cout << "RAR decoded successfully, TBS=" << rar_grant_nbytes << std::endl;

  // Set Backoff parameter
  if (rar_pdu_msg.has_backoff()) {
    backoff_param_ms = backoff_table[rar_pdu_msg.get_backoff() % 16];
    rDebug("RAR contains backoff parameter: %d ms", backoff_param_ms);
    std::cout << "RAR contains backoff parameter: " << backoff_param_ms << " ms" << std::endl;
  } else {
    backoff_param_ms = 0;
  }
  ...
  while (rar_pdu_msg.next()) {
    if (rar_pdu_msg.get()->has_rapid() && rar_pdu_msg.get()->get_rapid() == sel_preamble) {
      ...
      uint32_t ta_cmd = rar_pdu_msg.get()->get_ta_cmd();
      rDebug("Found matching RAPID=%d, TA=%d, T-CRNTI=0x%x",
             sel_preamble.load(),
             ta_cmd,
             rar_pdu_msg.get()->get_temp_crnti());
      std::cout << "Found matching RAPID=" << sel_preamble.load() 
                << ", TA=" << ta_cmd 
                << ", T-CRNTI=0x" << std::hex << rar_pdu_msg.get()->get_temp_crnti() << std::dec << std::endl;
      ...
      if (rar_pdu_msg.get()->has_rapid()) {
        ...
        std::cout << "Found RAR for preamble " << rar_pdu_msg.get()->get_rapid() << std::endl;
      }
    }
  }
}
