...
static uint32_t dci_nr_format_1_1_sizeof(const srsran_dci_cfg_nr_t* cfg, srsran_rnti_type_t rnti_type)
{
  ...
  // printf("after rm_pattern, count: %d\n", count);

  // ZP CSI-RS trigger - 0, 1, or 2 bits
  count += (int)SRSRAN_CEIL_LOG2(cfg->nof_aperiodic_zp + 1);
  // printf("after ZP CSI-RS, count: %d\n", count);

  // For transport block 1:
  // Modulation and coding scheme – 5 bits
  count += 5;
  // printf("after modulation, count: %d\n", count);

  // New data indicator – 1 bit
  count += 1;
  // printf("after data indicator, count: %d\n", count);

  // Redundancy version – 2 bits
  count += 2;
  // printf("after redundancy, count: %d\n", count);
  ...
  // printf("after 2cw, count: %d\n", count);

  // HARQ process number – 4 bits
  count += 4;
  // printf("after harq, count: %d\n", count);
  ...
  // printf("after harq codebook, count: %d\n", count);


  // TPC command for scheduled PUCCH – 2 bits
  count += 2;
  // printf("after tpc, count: %d\n", count);

  // PUCCH resource indicator – 3 bits
  count += 3;
  // printf("after PUCCH, count: %d\n", count);

  // PDSCH-to-HARQ_feedback timing indicator – 0, 1, 2, or 3 bits
  count += (int)SRSRAN_CEIL_LOG2(cfg->nof_dl_to_ul_ack);
  // printf("after PDSCH-to-HARQ_feedback timing indicator, count: %d\n", count);

  // Antenna port(s) – 4, 5, or 6 bits
  count += dci_nr_dl_ports_size(cfg);
  // printf("after antenna port, count: %d\n", count);

  // Transmission configuration indication – 0 or 3 bits
  if (cfg->pdsch_tci) {
    count += 3;
  }
  // printf("after tci, count: %d\n", count);
  ...
  // printf("after cbg, count: %d\n", count);

  // CBG flushing out information (CBGFI) – 0 or 1 bit
  if (cfg->pdsch_cbg_flush) {
    count += 1;
  }
  // printf("after flush, count: %d\n", count);

  // DMRS sequence initialization – 1 bit
  count += 1;
  // printf("after all, count: %d\n", count);
