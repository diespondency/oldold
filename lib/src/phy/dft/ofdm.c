...
static int ofdm_init_nr_nrscope(srsran_ofdm_t* q, srsran_ofdm_cfg_t* cfg, srsran_dft_dir_t dir, int scs_idx)
{
  ...
  // int cp1 = SRSRAN_CP_ISNORM(cp) ? SRSRAN_CP_LEN_NORM_NR(0, symbol_sz) : SRSRAN_CP_LEN_EXT(symbol_sz);
  // int cp2 = SRSRAN_CP_ISNORM(cp) ? SRSRAN_CP_LEN_NORM(1, symbol_sz) : SRSRAN_CP_LEN_EXT(symbol_sz);
  int cp1 = SRSRAN_CP_ISNORM(cp) ? SRSRAN_CP_LEN_NORM_NR(symbol_sz) : SRSRAN_CP_LEN_EXT(symbol_sz);
  printf("cp1: %d\n", cp1);
  // Slides DFT window a fraction of cyclic prefix, it does not apply for the inverse-DFT
  if (isnormal(cfg->rx_window_offset)) {
    cfg->rx_window_offset = SRSRAN_MAX(0, cfg->rx_window_offset);   // Needs to be positive
  ...
  // set phase compensation
  if (srsran_ofdm_set_phase_compensation_nrscope(q, cfg->phase_compensation_hz, scs_idx) < SRSRAN_SUCCESS) {
    ERROR("Error setting phase compensation");
    return SRSRAN_ERROR;
  }
...
int srsran_ofdm_set_phase_compensation_nrscope(srsran_ofdm_t* q, double center_freq_hz, int scs_idx)
{
  // Validate pointer
  if (q == NULL) {
    return SRSRAN_ERROR_INVALID_INPUTS;
  ...
  // Extract modulation required parameters
  uint32_t symbol_sz = q->cfg.symbol_sz;
  double   scs       = (double)SRSRAN_SUBC_SPACING_NR(scs_idx); //< Assume 15kHz subcarrier spacing
  double   srate_hz  = symbol_sz * scs;
  printf("symbol_sz: %u\n", symbol_sz);
  printf("srate_hz: %lf\n", srate_hz);
  ...
  for (uint32_t l = 0; l < q->nof_symbols; l++) {
    uint32_t cp_len =
        SRSRAN_CP_ISNORM(q->cfg.cp) ? SRSRAN_CP_LEN_NORM_NR(symbol_sz) : SRSRAN_CP_LEN_EXT(symbol_sz);
...
static void ofdm_rx_slot_nrscope(srsran_ofdm_t* q, int slot_in_sf, int coreset_offset_scs)
{
  ...
  for (int i = 0; i < q->nof_symbols; i++) {
    ...
    if (isnormal(q->cfg.phase_compensation_hz)) {
      ...
      // Apply correction
      // the phase_compensation is a scalar value, so we don't need to adjust the mulitplication 
      // after frequency resources collection
      srsran_vec_sc_prod_ccc(output, phase_compensation, output, nof_re);
    } else if (q->fft_plan.norm) {
      srsran_vec_sc_prod_cfc(output, norm, output, nof_re);
    }
